...
module LemonWayClient
  class AccountKycStatus
    ...
    # Status of the Payment account.  The list of status is available in annex .
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'ibans')
        if (value = attributes[:'ibans']).is_a?(Array)
          self.ibans = value
        end
      end

      if attributes.has_key?(:'sddMandates')
        if (value = attributes[:'sddMandates']).is_a?(Array)
          self.sdd_mandates = value
        end
      end
    end
    ...
    def hash
      [id, status, date, documents, ibans, sdd_mandates].hash
    end
