...
module LemonWayClient
  class MoneyInIDealInitInput
    ...
    # ID of iDEAL Issuer resource:  1. 21 for Rabobank.  2. 31 for ABN Amro.  3. 721 for ING.  4. 751 for SNS Bank.  5. 161 for Van Lanschot Bankiers.  6. 511 for Triodos Bank.  7. 761 for ASN Bank.  8. 771 for RegioBank.  9. 801 for Knab Bank.  10. BUNQNL2A pour Bunk Bank.
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'issuerId')
        self.issuer_id = attributes[:'issuerId']
      end
    ...
    def list_invalid_properties
      ...
      if @issuer_id.to_s.length > 10
        invalid_properties.push('invalid value for "issuer_id", the character length must be smaller than or equal to 10.')
      end

      if @issuer_id.to_s.length < 0
        invalid_properties.push('invalid value for "issuer_id", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @issuer_id.to_s.length > 10
      return false if @issuer_id.to_s.length < 0
    ...
    def issuer_id=(issuer_id)
      ...
      if issuer_id.to_s.length > 10
        fail ArgumentError, 'invalid value for "issuer_id", the character length must be smaller than or equal to 10.'
      end

      if issuer_id.to_s.length < 0
        fail ArgumentError, 'invalid value for "issuer_id", the character length must be great than or equal to 0.'
      end
    ...
    def hash
      [return_url, issuer_id, account_id, total_amount, commission_amount, comment, auto_commission].hash
    end
