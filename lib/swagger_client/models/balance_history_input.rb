...
module SwaggerClient
  class BalanceHistoryInput
    # Request balance at at given time in UTC Unix timestamp.
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'atDate')
        self.at_date = attributes[:'atDate']
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@at_date.nil? && @at_date !~ Regexp.new(/^-?\\d{1,12}$/)
        invalid_properties.push('invalid value for "at_date", must conform to the pattern /^-?\\d{1,12}$/.')
      end
    ...
    def valid?
      return false if !@at_date.nil? && @at_date !~ Regexp.new(/^-?\\d{1,12}$/)
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] at_date Value to be assigned
    def at_date=(at_date)
      if !at_date.nil? && at_date !~ Regexp.new(/^-?\\d{1,12}$/)
        fail ArgumentError, 'invalid value for "at_date", must conform to the pattern /^-?\\d{1,12}$/.'
      end
